...
{
    uint8_t regAddr[HX_IMU_I2C_ADDR_BYTE]  = {LSM9DS1_OUT_X_G};
    uint8_t rBuffer[6] = {0x00};
    int32_t retI2C = 0;
    int16_t gx_raw, gy_raw, gz_raw;
    ...
    gz_raw = rBuffer[4] | (rBuffer[5] << 8);
   	...
   	*x = (float)gx_raw *2000 / 32768.0;
    *y = (float)gy_raw *2000 / 32768.0;
    *z = (float)gz_raw *2000 / 32768.0;
...
{
    char string_buf[100] ;
    sprintf(string_buf, "%d | %c%1d.%3d | %c%1d.%3d | %c%1d.%3d\n", 
		num,accel_x.symbol, accel_x.int_part, accel_x.frac_part, 
